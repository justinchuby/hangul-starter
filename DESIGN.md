...
The “拼读小桌” is the primary learning surface. Its full-width result strip precedes the controls and holds the syllable, spelling romanization, selected final, cautious coda-sound approximation, live selection status, and replay control. Semantic fieldsets then separate all 19 modern Hangul onset consonants, all 21 modern vowels, common optional finals, and advanced final clusters. Wide screens use three equal control columns—onset, vowel, and final—while narrower screens stack them in learning order. Compact Korean-letter buttons expose `aria-pressed` and turn amber when hovered or selected. All 28 standard final indices are available: common single finals remain a first group and the 11 clusters are always visible in a compact second grid. A user-initiated change computes a real precomposed Hangul syllable with `0xAC00 + ((initial * 21 + vowel) * 28) + final` and uses the existing Korean speech path to play that complete syllable once; initial rendering and programmatic state updates never autoplay.

The consonant area first shows a compact “字形家族” map of the five `훈민정음 해례` mother forms and their historical add-stroke relations: `ㄱ → ㅋ`, `ㄴ → ㄷ → ㅌ`, `ㄴ → ㄹ`, `ㅁ → ㅂ → ㅍ`, `ㅅ → ㅈ → ㅊ`, and `ㅇ → ㆆ → ㅎ`. The obsolete `ㆆ` appears only to explain the historical path to `ㅎ`; it is not taught as a modern standard letter. Modern doubled forms `ㄲ、ㄸ、ㅃ、ㅆ、ㅉ` are separately marked as tense, not as a simple next add-stroke level. The adjacent “解例的设计地图” uses original, decorative mouth-position sketches for `ㄱ、ㄴ、ㅁ、ㅅ、ㅇ`, with an explicit left-front/right-back legend, and distinguishes real articulatory cues from the lower “字形和声音小贴士” mnemonics. Visible copy treats romanization as an entry-level cue and says aspiration and tense are neither exact Chinese/English sounds nor repeated reading.
...
Each of the 40 letter cards is a compact, single-layer pale-paper note with a large Korean glyph, a visibly marked approximate romanization cue, and one concise articulation, shape, or mouth-action hint. Vowels are organized as ten modern base vowels and eleven combined/diphthong vowels, with the historical heaven `ㆍ`, earth `ㅡ`, person `ㅣ` design logic stated as context rather than a modern pronunciation rule. A seamless, full-card playback button contains the learning content; an adjacent compact 44px learned-state chip independently toggles progress. Cards have restrained soft shadows and a small hover lift; they do not imply navigation. Pressing the learned-state chip updates `aria-pressed` and its spoken label, persists the letter set in `localStorage`, and updates the header count and native progress element. Resetting progress requires confirmation and announces the result through a polite live region.
...
- **SpeechSynthesis:** the page uses one Korean-voice lookup and one shared speaking path for the practice “听一听” control and each letter card’s playable main body. Every card has two adjacent native buttons: the main body plays its example, while the bottom “标为已学会” action independently toggles progress. Consonants are spoken as `ㅡ`-padded examples and vowels as silent-initial `ㅇ` syllables, never as isolated-jamo pronunciation claims. Support text distinguishes unavailable synthesis, no Korean voice, available device-based playback, playback in progress, completion, and playback failure; it refreshes on `voiceschanged`.
